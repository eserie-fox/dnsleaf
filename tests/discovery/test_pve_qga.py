...
def test_pve_qga_backend_skips_invalid_interfaces_and_address_items(caplog) -> None:
        ...
        "result": [
            {
                ...
                "ip-addresses": [
                    {"ip-address-type": "ipv4", "ip-address": "93.184.216.34", "prefix": 32},
                    {"ip-address-type": "ipv6", "ip-address": "2408:8266:5003:506a::458"},
                    "broken-address",
                ],
            },
            "broken-interface",
            ...
            },
            {
                ...
                "ip-addresses": [
                    {
                        "ip-address-type": "ipv6",
                        "ip-address": "2408:8266:5003:506a::999",
                        "prefix": 128,
                    }
                ],
            },
    ...
    with caplog.at_level(logging.WARNING, logger="arbor_ddns.discovery.parser"):
        result = backend.discover(TargetRef(kind=TargetKind.VM, id=201))

    assert result.error is None
    assert [candidate.cidr for candidate in result.candidates] == [
        "93.184.216.34/32",
        "2408:8266:5003:506a::999/128",
    ]
