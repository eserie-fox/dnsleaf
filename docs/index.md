# Documentation

- [README: installation and daily use](../README.md)
- [Architecture](architecture.md)
- [Configuration and workspace files](configuration.md)
- [Entry management](entry_management.md)
- [Discovery and selection](discovery.md)
- [DNS synchronization and prune safety](dns_sync.md)
- [systemd lifecycle](systemd.md)
- [Development rules and checks](development.md)
- [Release preparation](release.md)
- [Pending 1.1.0 release notes](release-notes/1.1.0.md)
- [Changelog and pre-rename internal history](../CHANGELOG.md)
