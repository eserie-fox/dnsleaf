"""Shared arbor-ddns logging configuration models and helpers."""
...
DEFAULT_LOG_FORMAT = "%(asctime)s level=%(levelname)s name=%(name)s %(message)s"
...
class ResolvedArborDDNSLoggingConfig(BaseModel):
    """Runtime-resolved arbor-ddns logging config."""
...
class ArborDDNSLoggingConfig(BaseModel):
    """Shared arbor-ddns logging schema."""
    ...
    def _validate_format(cls, value: str) -> str:
        ...
        if not stripped:
            raise ValueError("arbor_ddns_logging.format must not be blank")
        return stripped
    ...
    def resolve(
        ...
        return ResolvedArborDDNSLoggingConfig(
            level=self.level,
            format=self.format,
