...
from dnsleaf.logging.config import (
    LoggingStream,
...
class DailySymlinkFileHandler(logging.Handler):
    ...
    def __init__(
        ...
        *,
        ...
        self.stem = path.stem
        self.retention_days = (
            OutsideWorkspaceConfig.from_defaults().dnsleaf_logging.retention_days
...
def configure_default_logging(
    *,
    stream_name: LoggingStream | None = None,
    level: int | None = None,
    fmt: str | None = None,
) -> None:
    """Configure simple root logging for non-workspace commands."""

    defaults = OutsideWorkspaceConfig.from_defaults().dnsleaf_logging
    apply_logging_config(
        ResolvedLoggingConfig(
            level=defaults.resolved_level() if level is None else level,
            format=defaults.format if fmt is None else fmt,
            file_path=None,
            retention_days=defaults.retention_days,
            stream=defaults.stream if stream_name is None else stream_name,
        ),
        close_existing=True,
    )
...
def load_workspace_logging_config(
    ...
    *,
    ...
    logging_config = loaded_workspace.resolved_workspace.dnsleaf_logging
    file_path = (
