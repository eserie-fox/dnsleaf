...
from dnsleaf.dns.models import DesiredRecord, DNSRecord, PlannedChange, ProviderVerification
...
class CloudflareProviderConfig(BaseModel):
    ...
    def resolved_api_token(self) -> str:
        ...
        if not token:
            raise CloudflareConfigurationError(f"Cloudflare API token file is empty: {token_path}")
        return token
...
class CloudflareDNSProvider(DNSProvider):
    ...
    def _request_json(
        ...
        *,
        ...
        try:
            ...
            raise CloudflareNetworkError("failed to reach Cloudflare API") from None
    ...
    def _raise_api_error(
        ...
        message = _format_cloudflare_errors(payload).replace(self._api_token, "[redacted]")
